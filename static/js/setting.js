...
function player(config){
    ...
    if(config.url.indexOf(".m3u8")>0||config.url.indexOf(".mp4")>0||config.url.indexOf(".flv")>0||config.url.indexOf(".webm")>0){
        console.log('picture: ',config.pic);
        MPlayer(config.url,config.title,config.vkey,config.next,config.pic);
    }else{
...
/*播放器初始化*/
function MPlayer(url,tittle,vkey,nexturl,pic){
    $("#loading").remove();
    if(!nexturl){
        playcss(1);
    }else{
    ...
    var playerConfig={
        ...
        src:url,  /*视频播放地址*/
        title: tittle,/*视频标题*/
        poster: pic?pic:'mui/imges/background.jpg',/*背景图片*/
        autoplay: true,/*自动播放*/
    ...
    //    弹出层提示
        ...
        //mp.showToast('网页端弹幕已关闭，请下载APP发送', 6000)
    });
